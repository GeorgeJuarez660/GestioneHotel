/*
 * Script della pagina login-dipendente.html.
 * Estratto dal template: richiede common.js, caricato prima.
 */

const form = document.getElementById('formLoginDipendente');
const esito = document.getElementById('esito');
const btnSubmit = document.getElementById('btnSubmit');
const PAGINA_OSPITE = API_BASE + '/homepage/dipendente/';
...
async function gestisciLogin(event) {
  ...
  const codiceDipendente = document.getElementById('codiceDipendente').value.trim();

  const params = new URLSearchParams({
    username: username,
    password: password,
    codDipendente: codiceDipendente
  });
  ...
  try {
    const response = await fetch(API_BASE + '/login/dipendente?' + params.toString(), {
      method: 'GET'
    });
    ...
    if (response.ok) {
      ...
      window.location.href = '/hotel/homepage/dipendente/';
    } else {
